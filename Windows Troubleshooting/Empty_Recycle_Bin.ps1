...
function Force-RemoveRecycleBinFolders {
  param([switch]$WhatIfMode)
...
if (-not (Get-Command Clear-RecycleBin -ErrorAction SilentlyContinue)) {
  ...
  Force-RemoveRecycleBinFolders -WhatIfMode:$WhatIfMode
} else {
  Run-ClearRecycleBin -WhatIfMode:$WhatIfMode
